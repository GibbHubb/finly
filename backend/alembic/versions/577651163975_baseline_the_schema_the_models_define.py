...
def upgrade() -> None:
    # F34 — `CURRENT_TIMESTAMP`, not `now()`.
    #
    # Autogenerate emitted `server_default=sa.text('now()')` because it was run
    # against Postgres. CI runs `alembic upgrade head` against SQLite, which has
    # no `now()` — so the migration was portable exactly nowhere except the
    # database it was generated on. CURRENT_TIMESTAMP is standard and both
    # accept it.
